...
other_sex <- never %>%
  transmute(
    ...
    sex = sex %>%
      ...
      str_replace("f", "Women"),
    prop_childless_other_rex = p_childless / 100
  ) %>%
  left_join(
    never %>%
      select(country, sex, p_childless) %>%
      mutate(p_childless = p_childless / 100)
  )

never %>%
  mutate(
    prop_childless = p_childless / 100,
    prop_neverinunion = p_niu / 100
  ) %>%
  ggplot(aes(prop_childless, country)) +
  geom_point(size = 4, color = "#004444") + # this one is needed to set the coordinate space
  ...
  geom_segment(
    data = other_sex,
    aes(x = prop_childless_other_rex, xend = p_childless, yend = country),
    size = .5,
    color = "#004444AA"
  ) +
  geom_point(
  ...
  # ) +
  geom_moon(
    aes(ratio = prop_neverinunion, fill = sex, right = FALSE),
    size = 4,
    color = NA
  ) +
  facet_wrap(~sex, nrow = 1) +
...
# no title export
(out_no_tit <- ggdraw(
  main + theme(plot.title = element_blank())
) +
  draw_plot(inset, x = .15, width = .4, y = -.05, height = .3) +
  draw_plot(legend, x = .1, width = .5, y = .33, height = .25))
