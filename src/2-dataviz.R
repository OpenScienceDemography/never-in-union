...
gap_colors <- c("#00d5e9", "#ff5872","#7feb02", "#E37900FF", "#C7B40BFF",  "#3B90FFFF")
...
never %>% 
  ...
  geom_text(
    data = . %>% filter(sex == "Women"),
    x = 41, aes(label = country %>% tolower, color = continent), 
    size = 4, hjust = 1, family = "ah", fontface = 2,
    # color = "#004444AA"
  )+
  ...
  geom_text(
    ...
    aes(label = sign),
    x = c(33, 5), y = 75,
    size = 20, hjust = 0, colour = c("#1B5E20", "#4FC3F7"), 
    family = "Roboto", fontface = 2
  )
...
world_outline_robinson %>% 
  ...
  geom_sf(data = country_borders, color = "#ccffff", linewidth = .25)+
...
fig3main <- never_edu2 %>% 
  ...
  geom_text(
    ...
    aes(label = sign),
    x = .05, y = 90,
    size = 20, hjust = 0, colour = c("#1B5E20", "#4FC3F7") |> rep(each = 2), 
    family = "Roboto", fontface = 2
  )+
  geom_text(
    data = tibble(
    ...
    x = .8, y = 90,
    size = 7, hjust = 1, colour = c("#1B5E20", "#4FC3F7") |> rep(each = 2), 
    family = "Roboto", fontface = 2
  )
